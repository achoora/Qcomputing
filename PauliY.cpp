...
double imag=1;
  ...
  std::complex<double> PauliYGate[2][2]={{0,-iota},{iota,0}};
  for (std::complex<double> firstrow:PauliYGate[0])
  cout<<firstrow<<endl;
  for (std::complex<double> secondrow:PauliYGate[1])
  cout<<secondrow<<endl;
  int i,j,k;
...
for(i=0;i<2;i++)
{
  for(j=0;j<2;j++)
  {
  output[0][i]+=input[0][j]*PauliYGate[j][i];
  }
