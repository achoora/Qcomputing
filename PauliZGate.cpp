...
double imag=1;
  ...
  std::complex<double> PauliZGate[2][2]={{1,0},{0,-1}};
  for (std::complex<double> firstrow:PauliZGate[0])
  cout<<firstrow<<endl;
  for (std::complex<double> secondrow:PauliZGate[1])
  cout<<secondrow<<endl;
  int i,j,k;
...
for(i=0;i<2;i++)
{
  for(j=0;j<2;j++)
  {
  output[0][i]+=input[0][j]*PauliZGate[j][i];
  }
